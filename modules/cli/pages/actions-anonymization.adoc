...
For example, if you have a `REGEX_REPLACE` action followed by a `REPLACE` action, the `REPLACE` action will be executed on the output of the `REGEX_REPLACE` action like in the following configuration:
